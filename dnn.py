...
class dnn:
    ...
    def train(self, trainLabels):
        ...
        outputRef = wordUtil.WORD_CLASS_LABELS[trainLabelsArray[sntncIndex, wordIndex]]
        outputClassRef = wordUtil.WORD_CLASSES[trainLabelsArray[sntncIndex, wordIndex]]
        lineIn_h = self.lastHiddenOut
        train_models = []
        for i in xrange( self.bpttOrder ):
            startIndex = T.maximum( 0, wordIndex - self.bpttOrder + 1 )
            lineIn_i = (wordUtil.WORD_VECTORS[trainFeatsArray[sntncIndex, startIndex + i] : trainFeatsArray[sntncIndex, startIndex + i] + 1]).T # .T means transpose
            ...
            weightMatrix_o = self.weightMatrices[2 * self.bpttOrder][wordUtil.WORD_CLASS_CUM_SIZES[outputClassRef] : wordUtil.WORD_CLASS_CUM_SIZES[outputClassRef + 1]]
            weightMatrix_c = self.weightMatrices[2 * self.bpttOrder + 1]
            lineOutput_o = T.dot(weightMatrix_o, lineIn_h)
            lineOutput_c = T.dot(weightMatrix_c, lineIn_h)
            outputVector = ( T.nnet.softmax(lineOutput_o.T) ).T # .T means transpose
            outputClassVector = ( T.nnet.softmax(lineOutput_c.T) ).T # .T means transpose
            # cost = shared(0.)
            ...
            cost = - T.log(outputVector[outputRef, 0]) - T.log(outputClassVector[outputClassRef, 0])
            if (i == 0):
            ...
            train_model = function(inputs=[sntncIndex, wordIndex], outputs=[outputVector, outputClassVector, cost], updates=self.update(cost, i, lastHiddenOutUpdate, T.eq(wordIndex, maxLength - 1)))
        ...
        for epoch in xrange(self.epochNum):
            ...
            for i in xrange(numOfBatches): #feats and labels are shuffled, so don't need random index here
                ...
                for j in xrange( max([ trainSntncLengths[index] for index in shuffledIndex[i*self.batchSize : (i+1)*self.batchSize] ]) ):
                    self.wordIndex = j
                    self.out, self.outClass, self.cost = ( train_models[min(j, self.bpttOrder - 1)] )(shuffledIndex[i], j)
                    # self.out, self.outClass, self.cost = ( train_models[min(j, self.bpttOrder - 1)] )(shuffledIndex[i*self.batchSize : (i+1)*self.batchSize], j)
    ...
    def test(self, testLabels):
        ...
        self.lastHiddenOut = self.initLastHiddenOut()
        test_model, maxLength, testSntncLengths = self.getForwardFunction(testLabels, 1, self.weightMatrices)
        ...
        for j in xrange(len(testLabels)):
            for i in xrange(testSntncLengths[j]):
                outputVector, outputClassVector = test_model(j, i)
                sntncProbs[j] *= outputVector[wordUtil.WORD_CLASS_LABELS[testLabels[j][i]].eval(), 0] * outputClassVector[wordUtil.WORD_CLASSES[testLabels[j][i]].eval(), 0]
                print 'sntncProbs: ', sntncProbs[j]
    ...
    def getForwardFunction(self, testLabels, batchSize, weightMatrices):
        ...
        outputRef = wordUtil.WORD_CLASS_LABELS[testLabelsArray[sntncIndex, wordIndex]]
        # outputRef = testLabelsArray[sntncIndex * batchSize : (sntncIndex+1) * batchSize, wordIndex]
        outputClassRef = wordUtil.WORD_CLASSES[testLabelsArray[sntncIndex, wordIndex]]
        lineIn_h = self.lastHiddenOut
        
        lineIn_i = (wordUtil.WORD_VECTORS[testFeatsArray[sntncIndex, wordIndex] : testFeatsArray[sntncIndex, wordIndex] + 1]).T # .T means transpose
